...
7、本示例中 K 为套娃配置（选配），由 M 回落 VLESS+XHTTP 实现，即 VLESS+XHTTP 配置由 H 提供、REALITY 配置由 M 提供。
...
9、本示例中 F 兼容原版 Trojan 应用，即可使用原版 Trojan 客户端连接；但原版 Trojan 客户端不支持指纹伪造，故不推荐使用。
